...
public class TcpVATunnel extends VirtualGatewayTunnel {
    ...
    public TcpVATunnel(Session session, NioTunnel proxyServerTunnel, NioTunnel remoteServerTunnel, int mtu) {
        ...
        Request request = new Request(mRemoteTunnel);
        // Let injectors write fake responses back through the proxy tunnel
        request.setResponseTunnel(mProxyTunnel);
        this.mGateway = new NetBareVirtualGateway(session, request,
    ...
    public void connect(InetSocketAddress address) throws IOException {
        ...
        // Start reading from proxy tunnel immediately, don't wait for remote connect.
        // This allows the interceptor chain (sniffRequest etc.) to fire even when
        // the remote server is unreachable — the TCP connect may take 30-75s to timeout.
    ...
    private void setCallbacks() {
        ...
        mRemoteTunnel.setNioCallback(new NioCallback() {
            ...
            public void onConnected() throws IOException {
                // Proxy tunnel is already prepared for reading in connect().
                // Only prepare remote tunnel if it actually connected.
                boolean remoteFailed = (mRemoteTunnel instanceof TcpRemoteTunnel)
                        && ((TcpRemoteTunnel) mRemoteTunnel).isConnectFailed();
                if (!remoteFailed) {
