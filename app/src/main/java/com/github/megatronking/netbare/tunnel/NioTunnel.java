...
public abstract class NioTunnel<T extends AbstractSelectableChannel, S> implements Closeable,
    ...
    /**
     * Notify the callback that the connection event occurred.
     * Used by subclasses to invoke the callback even when connect fails.
     */
    protected void notifyConnected() throws IOException {
        if (mCallback != null) {
            mCallback.onConnected();
        }
    }
